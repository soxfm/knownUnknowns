...
__Modern developers often use **Ed25519 signatures** instead of **256-bit curve ECDSA** signatures, because EdDSA-Ed25519 signature scheme uses keys, which fit in 32 bytes \(64 hex digits\), signatures fit in 64 bytes \(128 hex digits\), signing and verification is faster and the security is considered better.__
